...
Part 1 captured what 50 large U.S. companies **said they valued** on their public
...
and aimed at the people who can fire the board. The question is simple: do the values a
company markets show up where it answers to its owners?
...
> `PART2_SUMMARY.md` is the non-technical summary; `PART2_SLICE_FINDINGS.md` is the
> earlier 3-company sanity-check record (superseded). I validated the pipeline on a
> sector-spanning slice first, then scaled it unchanged.
...
6. **Analyze** (the real work): classical NLP on the full text plus selective LLM tagging on
   a values excerpt, producing a documented structured dataset and aggregate trend tables.
...
- **Robust filing selection on contested years.** In a proxy fight the subject company's
  ...
  candidates (real annual form first, then longest cleaned text, since the full statement
...
- **Classical first, LLM for the residual judgment.** Dictionary counts / tf-idf run on
  ...
  on what counts can't do, namely semantic theme presence and shareholder-vs-stakeholder
  framing, on a bounded values excerpt, cached by content hash. (Full justification in the
  schema.)
...
- **Cleaned full text is the analysis surface.** We keep the whole proxy, including the
  ...
  tf-idf `min_df`), rather than trying to segment sections. It's simpler and reproducible.
...
- **Direct Part-1↔Part-2 join + gap score.** Per company-year, set the proxy's
  theme/stakeholder profile against Part 1's stated-values profile and compute a "say-do"
  divergence. This is the seed of the authenticity index (Parts 3–4).
