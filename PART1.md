...
against the live archive, and the redesign is shaped by what CDX actually does:
...
- **Upstream reality + the gentle re-run.** CDX latency is highly variable and the archive
  ...
  known-good URL returned 0 captures — a pure throttling artifact, not a real miss. The fix is
...
exactly what is missing and why — and every blank for a big company must end up justified by
...
- **Agent targets**: every company with <9 years that is not a timeout — `genuine_miss`,
...
- **Canonical flag.** When only a sub-brand/regional page exists in the archive (e.g. XOM's
  `/aviation/about-us` — `corporate.exxonmobil.com` is empty in the archive), the agent fills
...
- **Cost dependency.** The agent is the only step that needs Anthropic API credits; everything
  ...
  agent is also the dominant cost — scope it tightly (only genuine gaps), and lean on the
...
accordingly — the archive is a shared public resource, so stay polite, ~6–10 in flight),
