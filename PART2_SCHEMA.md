...
| `filing_date` | str | Actual SEC filing date (YYYY-MM-DD). Provenance + lets a reader recover the meeting-year mapping. |
...
| `dominant_theme_distinctive` | str | Argmax **excluding** the two structural themes (`leadership_governance`, `profitable_growth`). The *discretionary* values emphasis — the informative version of "what this proxy foregrounds beyond its mandatory content." |

### Tone (classical) — LM-style, per 1,000 words
| column | type | why it's here |
|---|---|---|
| `tone_positive` | float | Positive-sentiment term frequency. Proxies are promotional; level + drift are signals. |
...
shows which value themes rose/fell in proxy language (compare to Part 1's trend).
...
`biggest_jump`, and `verdict` (`aligned` / `partial` / `weak` / `no`). Descriptive
...
- **Classical (full corpus, every filing):** theme frequencies, tone, readability,
  sizes, tf-idf similarity. Transparent, free, deterministic, identical on every rerun
  — the right tool for a 450-filing time series and for anything a keyword/vector can
  capture.
- **LLM (bounded excerpt, selective):** semantic theme presence and
  shareholder-vs-stakeholder framing — judgments that need reading comprehension, not
