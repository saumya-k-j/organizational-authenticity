...
a company's public website emphasizes each value than its proxy filings back up — graded by
degree, asymmetric (over-claiming is penalized, modesty is not), scored only where both
sides exist, with a **trajectory** showing whether the gap is widening or closing over
2016–2024.
...
versus **accountable self-presentation** (the proxy, Part 2). Everything below should be read
in that light.
...
`UnderClaimIndex` (discloses more than it advertises — modesty) is reported alongside but is
...
# Part B — How I reasoned to this (first-person)

<!-- TODO (Ssaumya): rewrite this whole section in my own voice before final submission. The reasoning is mine but the phrasing is drafted. Kill the em-dashes, cut the 'what nagged at me' / 'it came down to for me' narration, plainer sentences. -->

*This is the actual order I worked through it, doubts and course-corrections included — not a
tidied-up methodology section.*
...
filings, and average it out. But that felt too naive almost immediately — real life has too
many things going on for a flat average to mean much.

The first thing that bothered me was **missing data**. If a company has no website text for a
...
The bigger thing that kept nagging at me was **time**. A single per-year score is just a
...
needs a trajectory — is the gap widening or closing across 2016–2024 — not just a snapshot.
...
both as too simplistic — overlap treats a meaningful match and a throwaway match the same;
rank agreement felt mechanical. Neither captured what authenticity actually is.

What it came down to for me: **authenticity is about not claiming what you don't back up.** A
...
The hardest realization came when I asked whether I could actually validate the proxy side.
...
market, I'm mixing a text comparison with an economic model and can't defend either cleanly. A
measure should do one thing clearly. So confounders like market conditions go into the
limitations and interpretation, not the formula.
...
The last thing that caught me out was **evidence**. When I looked at which companies came out
...
In the end the index does one clear thing: per company-year, it measures emphasis-weighted
